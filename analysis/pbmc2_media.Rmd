...
**PBMC dataset** that has been collected to test the influence of media storage
on sequencing. It includes 11045 human PBMCs from the same sample, split into 3 batches. 
All batches have been sequenced together and libraries have been prepared together, 
but 2 batches have been stored in different media DMSO, MetOH and frozen before sequencing. 
The last batch contains PBMCs from freshly collected blood. 
Differences between these batches should all derive from media storage, so conditional sources.
...
# contrast
cont <- list("DMSO-fresh" = c(1,-1,0,0),
             "MetOH-Fresh" = c(0,-1,1,0),
             "DMSO-MetOH" = c(1,0,-1,0))
...
doDE <- function(x,lfc_cutoff=log2(1.1)){
  res<-lapply(kids, function (k) {
    ...
    es_tmp <- es[,n]
    grp <- group[n]
    patient<-sce$patient[n]
    design <- model.matrix(~0+grp+patient)
...
signs <- sample(c(-1, 1), nrow(lfc_dist), TRUE, prob = c(0.4, 0.6))
...
Xadj1 <- adj1(counts(sce), colData(sce)[, batch], colData(sce)[, celltype])
Xadj2 <- adj2(counts(sce), colData(sce)[, batch], colData(sce)[, celltype])
Xadj3 <- adj3(counts(sce), colData(sce)[, batch], colData(sce)[, celltype])
...
plot_pca(counts(sce)) + ggtitle("unadjusted")
...
#umap
plot_umap <- function(X, sce){
  assays(sce)$removed_batch <- X
  sce <- runUMAP(sce, ntop = 2000, exprs_values = "removed_batch")
  visGroup(sce, batch, dim_red = "UMAP")
}

plot_umap(Xadj1, sce) + ggtitle("constant batch effect")
plot_umap(Xadj2, sce) + ggtitle("constant batch effect, different ct composition")
plot_umap(Xadj3, sce) + ggtitle("celltype specific batch effect")
...
get_cms <- function(X, sce, method_nam){
  assays(sce)$removed_batch <- X
  sce <- runPCA(sce, ntop = 2000, exprs_values = "removed_batch", ncomponents = 10)
