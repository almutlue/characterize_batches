...
if(MultiSample){
  ...
  col_dat <- data.frame("sample_id" = col_tib[,sample], "group_id" = col_tib[,batch]) %>%
    set_rownames(levels(as.factor(colData(sce)[,sample])))
...
Calculate cms
```{r cms}
#sce <- cms(sce, group = batch, k = 300, k_min = 180, cell_min = 20, n_dim = 10)
#sce <- runUMAP(sce)
