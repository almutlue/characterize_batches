...
# sce <- runUMAP(sce)
# sce <- saveRDS(paste0(data_path, "/pbmc2_media.rds"))
...
nb<-match(rownames(sce@assays$data$logcounts),vp$gene)
vp1<-vp[nb,]
var_batch<-vp1[,batch]*var_genes
var_cell<-vp1[,celltype]*var_genes
```
...
m_batch <- mean(var_batch)
m_celltype <- mean(var_cell)
...
me_batch <- median(var_batch)
me_celltype <- median(var_cell)
...
de_be <- n_de %>% bind_cols() %>% mutate("fresh" = rowMeans(.[,c(1,2)]), "MetOH" = rowMeans(.[,c(2,3)]), "DMSO" = rowMeans(.[,c(1,3)]))
