...
### Variance partioining and gene expression

How strong is the variance partitioning affected by the overall expression of that gene? We check the realtion between gene expression and variance explained by batch/celltype.

```{r var dev expr}
#plot variance along with expression
rownames(vp) <- vp$gene
vp <- vp[rownames(sce),]
...
## Celltype abundance - differences in relative abundances

We determine the realtive abundances of each celltype per batch and compare relative abundances between batches.
...
Do the overall count distribution vary between batches? Do count distributions within celltypes vary by batch? Are there differences by different batches. Here we visualize count distributions to show how the batch effect manifests (e.g. by shifts in many genes, a few genes, systematic shifts ...) 
...
#plot mean expresiion by batch and celltype
...
### Compare mean expression batchwise for each celltype
...
This score gives us a way to estimate the degree of mixing of batches. the lower the score the less are cells mixed. The more cells with a score close to 0 the less are cells mxed and the "stronger" is the batch effect.
```{r cms}
...
Calculate DEG for each cluster separately using limma (similar to clusterwise t-tests). For each celltype we calculate differentially expressed genes separately to get celltype independent results. As we do not have replicates we consider each cell a single sample. 
```{r deg}
x <- sce
...
# Summarize DE genes
To compare different batch types we summarize main features of differntial expression analysis. The number of significant DE genes (p.adj < 0.05) per celltype as well as the mean DE genes over all celltypes.
The number of genes with a logFoldChange > 1 by celltype and the mean of all celltypes.
...
To simulate a realistic variance by celltype we model the lfc by celltypes from the real underlying distribution. Most lfc follow a gamma distribution with varying parameter, but there seem to be a set of outstanding genes with a distinct higher lfc. To model them we use a second distribution for these "type genes". We estimate the number of "type genes" by determining the number of genes high than the maximum (and lower than the minimum) of our fitted gamma distribution. We need this (at least for some batches) to cover the maxima in our simulation.

```{r lfc ct}
# Determine lfc by celltype by running a pairwise t-test for all cluster against each other
...
Again we model 2 lfc distributions (one for the fitted gamma distribution and a normal distribution with a higher mean) to cover the extremes as well. Most lfc follow a gamma distribution with varying parameter, but there seem to be a set of outstanding genes with a distinct higher lfc. To model them we use a second distribution for these "batch genes". We estimate the number of "batch genes" by determining the number of genes high than the maximum (and lower than the minimum) of our fitted gamma distribution.
```{r lfc batch}
...
Below we define three functions that we will use to remove the batch effect - the difference between them is which linear model we fit, and thus which residuals we use. Throughout, we will assume that X is a matrix with log-expression values, ct is a cell type vector, and bt is a batch vector.
...
plot_pca <-function(X){
  ...
  scale_shape_manual(values=1:nlevels(colData(sce)[, celltype]))
}
