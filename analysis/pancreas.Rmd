...
# sce <- runUMAP(sce)
# saveRDS(sce,paste0(data_path,"/",  "pancreas.rds"))
#sce<-readRDS(paste0(data_path,"/",  "pancreas.rds"))
# preprocessing step

# sce <- sce[which(rowSums(as.matrix(assays(sce)$counts) > 0) > 0), ]
# is.mito <- grepl("^MT", rownames(as.matrix(assays(sce)$logcounts)), ignore.case=T) 
# summary(is.mito) 
# 
...
# cct <- table(sce$isOutlier, sce$tech) 
