...
sce<-readRDS(paste0(out_path, '/sce_hca_integrated.rds'))
#sce<-runUMAP(sce)
#colors
...
```{r vis batch}
 ...
 lapply(feature_list, function(feature_name){
   visGroup(sce, batch, dim_red= "TSNE")
 })
...
# vp <- varPart
# saveRDS(vp, paste0("vp_", dataset_name, ".rds"))
...
```{r count distributions, fig.height=6, fig.width=7}
...
mean_list <- lapply(sids, function(batch_var){
  ...
  mean_c <- mean_cluster %>% purrr:::map(rowMeans) %>% bind_rows %>%
...
#sce$protocol <- as.factor(sce$protocol)
# sce <- cms(sce, k = 300, group = "protocol", dim_red = "PCA", cell_min = 30, n_dim = 10)

# saveRDS(sce, file = paste0(out_path, "/cms_4_mouse_red.rds"))

sce <- readRDS(paste0(out_path, "/cms_4_mouse_red.rds"))
# sce <- runUMAP(sce)
...
doDE <- function(x,lfc_cutoff=log2(1.1)){
  ...
  tt <- lapply(res, "[[", "tt")
  tt <- lapply(cs, function(c) purrr::map(tt, c))
...
saveRDS(res, file=paste0("limma_", dataset_name, ".rds"))
```
...
# m2<-readRDS(paste0( "upset_", dataset_name, ".rds"))


#lapply(m2, function(x) UpSet(x))

# unique DEGs for all clusters

DEG<-unique(unlist(result))
saveRDS(DEG,file=paste0('deg_',dataset_name,'.rds'))
```
