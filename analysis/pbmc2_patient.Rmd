...
Pbmc - patients storage

**PBMC dataset** that has been collected to test the influence of media storage
on sequencing. It includes 28296 human PBMCs from the same sample, split into 2 patients. Material from each patient have been stored in different media DMSO, MetOH and frozen before sequencing and the last batch contains PBMCs from freshly collected blood. 
...
#Deviance explained by Piere Luc 
```{r}
...
doDE <- function(x,lfc_cutoff=log2(1.1)){
  res<-lapply(kids, function (k) {
    ...
    es_tmp <- es[,n]
    grp <- group[n]
    media<-factor(sce$media[n])
    design <- model.matrix(~0+grp+media)
